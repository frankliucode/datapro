set underline ~
set echo on
set verify off
set pagesize 1000
set linesize 150

col rtime heading 'RUN|TIME'

col snap_id format 9999999;
col rtime format a20;
col name format a30;
col total_mb format 999999;
col used_mb format 999999;
col free_mb format 999999;
col pct format 99.9;

accept tbsp_name prompt 'enter tablespace name:'
accept snap_date prompt 'enter snapshot date (mm/dd/yyyy):'

variable b1 varchar2(30);
variable b2 varchar2(20);

begin
  :b1 := '&tbsp_name';
  :b2 := '&snap_date';
end;
/

select u.snap_id,
       u.rtime,
       ts.NAME,
       u.tablespace_size / 128 total_mb,
       u.tablespace_usedsize / 128 used_mb,
       (u.tablespace_size - u.tablespace_usedsize) / 128 free_mb,
       u.tablespace_usedsize * 100 / u.tablespace_size pct
  from dba_hist_tbspc_space_usage u
  join v$tablespace ts
    on ts.TS# = u.tablespace_id
 where ts.NAME = upper(:b1)
   and substr(u.rtime, 1, 10) = :b2
order by u.snap_id desc   
;



